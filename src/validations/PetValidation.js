...
const PetValidation = {
  create: [
    body('owner_id')
      ...
      .isInt().withMessage('Owner ID must be a valid integer'),
    body('category_id')
      .notEmpty().withMessage('Category ID is required')
      .isInt().withMessage('Category ID must be a valid integer'),
    body('name')
      .notEmpty().withMessage('Pet name is required')
      .isString().withMessage('Pet name must be a valid string'),
    body('breed')
      .optional()
      .isString().withMessage('Breed must be a valid string'),
    body('age')
      .optional()
      .isInt({ min: 0 }).withMessage('Age must be a non-negative integer'),
    body('gender')
      .optional()
      .isIn(['Male', 'Female', 'Unknown']).withMessage('Gender must be one of the following: Male, Female, Unknown'),
    body('description')
      .optional()
      .isString().withMessage('Description must be a valid string'),
    body('medical_history')
      .optional()
      .isString().withMessage('Medical history must be a valid string'),
  ],
  ...
  update: [
    ...
    body('gender').optional().isIn(['Male', 'Female', 'Unknown']).withMessage('Gender must be one of the following: Male, Female, Unknown'),
    body('description').optional().isString().withMessage('Description must be a valid string'),
    body('medical_history').optional().isString().withMessage('Medical history must be a valid string'),
  ],
