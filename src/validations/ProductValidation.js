...
const ProductValidation = {
  create: [
    body('sales_center_id')
      .notEmpty().withMessage('Sales Center ID is required')
      .isInt().withMessage('Sales Center ID must be a valid integer'),
    body('category_id')
    ...
    body('sku')
      .optional()
      .isString().withMessage('SKU must be a valid string'),
    body('images')
      .optional()
  ...
  update: [
    ...
    body('sales_center_id').optional().isInt().withMessage('Sales Center ID must be a valid integer'),
    ...
    body('stock_quantity').optional().isInt({ gt: -1 }).withMessage('Stock quantity must be a non-negative integer'),
  ...
  filterProducts: [
    ...
    query('min_price').optional().isFloat({ gt: 0 }).withMessage('Minimum price must be a positive number'),
    query('max_price').optional().isFloat({ gt: 0 }).withMessage('Maximum price must be a positive number'),
