...
const ServiceValidation = {
  ...
  update: [
    ...
    body('name').optional().isString().withMessage('Service name must be a valid string'),
    body('description').optional().isString().withMessage('Description must be a valid string'),
    body('price').optional().isFloat({ gt: 0 }).withMessage('Price must be a positive number'),
  ],

  getById: [param('id').isInt().withMessage('Service ID must be a valid integer')],

  delete: [param('id').isInt().withMessage('Service ID must be a valid integer')],
