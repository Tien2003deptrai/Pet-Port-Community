...
const PaymentValidation = {
  create: [
    ...
    body('items')
      .notEmpty().withMessage('Payment items are required')
      .isArray().withMessage('Payment items must be an array')
      .custom((value) => {
        if (!value.length) {
    ...
    body('items.*.name')
      ...
      .isString().withMessage('Item name must be a valid string'),
  ...
  updatePaymentStatus: [
    body('paymentIntentId')
      ...
      .isString().withMessage('Payment Intent ID must be a valid string'),
    body('status')
      .notEmpty().withMessage('Status is required')
      .isIn(['Pending', 'Completed', 'Failed']).withMessage('Status must be one of the following: Pending, Completed, Failed'),
    body('orderId')
      .notEmpty().withMessage('Order ID is required')
      .isInt().withMessage('Order ID must be a valid integer'),
  ],
